define({
  ...
  showAddNewPermissions : function(){
    ...
    //NOTE:Hiding assign roles tab for 2022.10
    this.view.flxAddRoles.setVisibility(false);
    this.view.btnAddPermissionNext.setVisibility(this.isKeyCloakEnabled === false);
    /**/
  ...
  showAddUsers : function(){
    ...
    this.hideMainSubHeader();
    this.view.flxAddMainContainer.setVisibility(true);
    //NOTE: hiding roles tab for 202210
    this.view.flxAddRoles.setVisibility(false);
    /**/
    this.view.flxAddUsers.setVisibility(this.isKeyCloakEnabled===true?false:true);
  ...
  showViewRoleSegmentAndPermissionHeader : function(){
   ...
   // NOTE: Hiding roles tab for 2022.10 as LE enhancements yet tobe done
    //this.getRolesDirectlyWithPermission(callBack);
    this.view.flxViewTab1.setVisibility(false);
    this.view.flxPermissionsHeader.setVisibility(false);
    this.view.flxViewSegmentAndHeaders.setVisibility(false);
   // this.tabUtilLabelFunction([this.view.lblTabName1,
                              // this.view.lblTabName2],this.view.lblTabName1);    
    if(this.isKeyCloakEnabled===false){
      this.showViewUsersSegmentAndHeader();
  ...
  showViewUsersSegmentAndHeader : function(){
    ...
    var callBack = function(users){
      ...
      self.view.flxViewSegmentAndHeaders.setVisibility(true);
  ...
  parseAllRolesData: function(allRoles){
    ...
    var data = allRoles.map(function(allRoles) {
      if(allRoles.legalEntitiesRoleInfo){
      for(var i=0;i<allRoles.legalEntitiesRoleInfo.length;i++){
      return{
        "btnAdd": {
          "text":kony.i18n.getLocalizedString("i18n.frmPermissionsController.Add"),
          "onClick":self.addPermissionstoRole
        },
        "lblPermissionsName": allRoles.legalEntitiesRoleInfo[0].role_Name,
        "rtxPermissionDescription": allRoles.legalEntitiesRoleInfo[0].role_Desc,
        "template":"flxAddPermissions",
        "permissionId":allRoles.legalEntitiesRoleInfo[0].role_id
      };
      }
      }
      else{
  ...
  setFlowActions : function(){
    ...
    this.view.btnAddPermissionNext.onClick= function(){
      //NOTE: changing the next button navigation as roles tab is hidden for 202210
     /* if(scopeObj.validatePermissionName()){
        scopeObj.showAddRoles();
        scopeObj.view.fontIconRightArrow4.setVisibility(true);
        scopeObj.showAddRoles();
  ...
  toPermissionSegment: function (permission) {
    ...
    return {
      ...
      // NOTE: Hiding roles label for 2022.10 as LE enhancements yet tobe done
      "lblNoOfRoles": {"isVisible":false},
  ...
  setPermissionSegmentData : function(permissions,isFilter){
    ...
    var self = this;
    //NOTE:hiding roles column for 202210
    this.view.flxRoles.setVisibility(false);
    /**/
...
downloadPermissionsList : function(){
  var scopeObj = this;
    ...
    var searchText = scopeObj.AdminConsoleCommonUtils.getEncodedTextInput(scopeObj.view.subHeader.tbxSearchBox.text);
     ...
     var downloadPermissionsFilterJSON = scopeObj.view.mainHeader.flxDropdownList.info;
  ...
  downloadCSV:function() {
    ...
    var downloadURL = mfURL + "/services/data/v1/RolesAndPermissionsObjService/operations/permissions_view/downloadPermissionsList" ;
     var searchText = scopeObj.AdminConsoleCommonUtils.getEncodedTextInput(scopeObj.view.subHeader.tbxSearchBox.text);
    if(searchText !== "") {
    ...
    var downloadPermissionsFilterJSON = scopeObj.view.mainHeader.flxDropdownList.info;
    if(downloadPermissionsFilterJSON !== undefined) {
      if(downloadPermissionsFilterJSON.selectedTypeList !== undefined) {
        var typeParam = paramCount > 0 ?  "&type=" : "?type=";
        downloadURL += typeParam + downloadPermissionsFilterJSON.selectedTypeList;
        paramCount = paramCount +1;
      }
      if(downloadPermissionsFilterJSON.selectedStatusList !== undefined) {
        var statusParam = paramCount > 0 ?  "&status=" : "?status=";
        downloadURL += statusParam + downloadPermissionsFilterJSON.selectedStatusList;
      }
    }
    this.callDownloadCSVService(downloadURL,"Permissions");
  },
  ...
  prefillData:function(){
    ...
    scopeobj.view.mainHeader.flxDropdownList.setVisibility(false);
  ...
  performStatusFilter: function () {
    ...
    if (indices !== null) { //show selected indices data
      ...
      if(self.view.mainHeader.flxDropdownList.info === undefined) {
        self.view.mainHeader.flxDropdownList.info = {};
      }
      self.view.mainHeader.flxDropdownList.info.selectedStatusList = statuses.substring(0, statuses.length-1);
