define([],function(){
	...
	var setterFunctions = {
		...
		resultAttributeValue : function(val, state){
			state['resultAttributeValue'] = val;
		},
	...
	function DecisionResult(defaultValues){
		var privateState = {};
			...
			privateState.resultAttributeValue = defaultValues?(defaultValues["resultAttributeValue"]?defaultValues["resultAttributeValue"]:null):null;
		...
		//Defining Getter/Setters
			Object.defineProperties(this,{
				...
				"resultAttributeValue" : {
					get : function(){return privateState.resultAttributeValue},
					set : function(val){
						setterFunctions['resultAttributeValue'].call(this,val,privateState);
					},
					enumerable : true,
				},
				"softdeleteflag" : {
	...
	BaseModel.isParentOf(DecisionResult);
	
	//Create new class level validator object
	BaseModel.Validator.call(DecisionResult);
	
	var registerValidatorBackup = DecisionResult.registerValidator;
	
	DecisionResult.registerValidator = function(){
		var propName = arguments[0];
		if(!setterFunctions[propName].changed){
			var setterBackup = setterFunctions[propName];
			setterFunctions[arguments[0]] = function(){
				if( DecisionResult.isValid(this, propName, val) ){
	...
	DecisionResult.relations = relations;
	
	DecisionResult.prototype.isValid = function(){
		return DecisionResult.isValid(this);
	};
	
	DecisionResult.prototype.objModelName = "DecisionResult";
